...
print("🔧 Đang thêm V14 vào dropdown của tất cả các file...\n")
...
for filename in files_to_update:
    try:
        ...
        # Tìm vị trí sau V13 entry
        pattern = r'(Gradient Icon</div>\s*</a>)'
        ...
        if re.search(pattern, content):
            ...
            if new_content != content:
                with open(filename, 'w', encoding='utf-8') as f:
                    f.write(new_content)
                print(f"✅ {filename} - Đã thêm V14")
            else:
                print(f"⚠️  {filename} - Không có thay đổi")
        else:
            print(f"⚠️  {filename} - Không tìm thấy V13 pattern")
