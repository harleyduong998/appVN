...
# Dropdown template chuẩn từ index_v11.html
dropdown_template = '''                <div id="versionDropdown"
...
print("🔧 Đang cập nhật dropdown cho tất cả các file...\n")

for filename, active_version in files_to_update:
    try:
        ...
        active_flags = {
            ...
            'v11_active': ' bg-slate-50' if active_version == 'v11' else '',
        }
        
        new_dropdown = dropdown_template.format(**active_flags)
        
        # Tìm và thay thế dropdown cũ
        pattern = r'<div id="versionDropdown".*?</div>\s*</div>\s*</div>'
        new_content = re.sub(pattern, new_dropdown, content, flags=re.DOTALL)
        
        if new_content != content:
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(new_content)
            print(f"✅ {filename} - Đã cập nhật dropdown")
        else:
            print(f"⚠️  {filename} - Không tìm thấy dropdown để thay thế")
...
print("\n🎉 Hoàn tất cập nhật tất cả dropdown!")
